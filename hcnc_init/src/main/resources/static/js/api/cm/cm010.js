...
function upsertUserBtn() {
    ...
    if (!userId) {
        showAlert({ // 알림(info), 경고(warning), 오류(error), 완료(success)
            icon: "warning",
            title: "경고",
            html: `<div><strong>아이디</strong>을(를) 입력해주세요.</div>`
        });
        $("#write_user_id").focus();
        return;
    }
    ...
    if (!userNm) {
        showAlert({ // 알림(info), 경고(warning), 오류(error), 완료(success)
            icon: "warning",
            title: "경고",
            html: `<div><strong>이름</strong>을(를) 입력해주세요.</div>`
        });
        $("#write_user_nm").focus();
        return;
    }
    ...
    if (!roleCd) {
        showAlert({ // 알림(info), 경고(warning), 오류(error), 완료(success)
            icon: "warning",
            title: "경고",
            html: `<div><strong>역할</strong>을(를) 선택해주세요.</div>`
        });
        $("#write_role_cd").focus();
        return;
    }
    ...
    if (currentMode === "insert" && !pwdHash) {
        showAlert({ // 알림(info), 경고(warning), 오류(error), 완료(success)
            icon: "warning",
            title: "경고",
            html: `<div><strong>비밀번호</strong>을(를) 입력해주세요.</div>`
        });
